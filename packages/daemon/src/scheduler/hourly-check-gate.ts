/**
 * Stage-1 deterministic gate for the three-stage hourly_check funnel
 * (cost-reduction-structural §B). Pure function over a `HourlyCheckSignals`
 * snapshot — no DB handle, no clock, no I/O. The dispatcher computes the
 * snapshot via `computeHourlyCheckSignals`, then asks this module which
 * stage to enter.
 *
 ...
 *   - `stage3`: existing full hourly_check session.
 *
...
import type { HourlyCheckSignals } from "../db/hourly-check-signals.js";

export type HourlyCheckGateStage = "stage0_silent" | "stage2" | "stage3";

export interface HourlyCheckGateConfig {
  /**
...
export interface HourlyCheckGateDecision {
  stage: HourlyCheckGateStage;
  /** Short, telemetry-friendly explanation. */
  reason: string;
  /** Snapshot at decision time (echoed for the audit row). */
  signals: HourlyCheckSignals;
}
...
export function decideStage(
  signals: HourlyCheckSignals,
  config: HourlyCheckGateConfig,
): HourlyCheckGateDecision {
...
export function buildGateAuditDetail(
  ...
  extra: {
    appliedDecision: HourlyCheckGateStage;
     ...
     */
    preEscalateGateStage?: HourlyCheckGateStage;
