/**
 * Stage-0 signal compute for the three-stage hourly_check gate
 ...
 * a pure function over `HourlyCheckSignals` + a config block.
 */
...
export interface HourlyCheckSignals {
  ...
  /**
   * Hours since the last hourly_check that escalated to Stage 3 (or
   * before the gate was deployed, since the last hourly_check run at
   * all). `Infinity` when no row has been recorded yet.
   */
  hoursSinceLastStage3Run: number;
}

export interface ComputeHourlyCheckSignalsOptions {
...
export function computeHourlyCheckSignals(
  db: Database.Database,
  options: ComputeHourlyCheckSignalsOptions = {},
): HourlyCheckSignals {
  ...
  // `HourlyCheckCoordinator.harvestForGate` structurally prevents a
...
function computeHoursSinceLastStage3(
  ...
  // Prefer the gate-emitted row (`hourly_check.gate` with
  // stage_reached='stage3') — that matches the design doc's "since the
  // last Stage 3 run" semantics. Fall back to the legacy
  // `routine.hourly_check` action-type for installs deployed before
  ...
  const gateRow = db
    .prepare(
      `SELECT started_at
         FROM agent_actions
        WHERE action_type = 'hourly_check.gate'
  ...
  if (!lastStarted) {
    const fallbackRow = db
      .prepare(
        `SELECT started_at
           FROM agent_actions
          WHERE action_type = 'routine.hourly_check'
