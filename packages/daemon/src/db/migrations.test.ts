...
// `0025-morning-today-budget-bump` — same CLAUDE.md non-negotiable #4
// contract as 0006 / 0009 / 0017: fresh DB (no table) → no-op + id recorded;
// fresh install already seeded at the new $2.00 → untouched; pre-migration
// preset-default row at $1.50 (claude/opencode) or $2.25 (codex/gemini
// scaled) → bumped to $2.00 / $3.00 + id recorded; operator-pinned ('user')
// or already-custom rows → untouched; sibling keys → untouched; re-run → no
// second bump.
describe("0025-morning-today-budget-bump", () => {
  const migration = MIGRATIONS.find(
    (m) => m.id === "0025-morning-today-budget-bump",
  );
  ...
  it("is a no-op when process_backend_config does not exist (fresh/empty DB)", () => {
    const db = openDb();
    const result = runMigrations(db, [migration!]);
    expect(result.applied).toEqual(["0025-morning-today-budget-bump"]);
    const recorded = db
      .prepare<[string], { id: string }>(
        "SELECT id FROM schema_migrations WHERE id = ?",
      )
      .get("0025-morning-today-budget-bump");
    expect(recorded).toEqual({ id: "0025-morning-today-budget-bump" });
  });

  it("is a no-op on a fresh install where the seed already wrote the new $2.00", () => {
    const db = openDb();
    seedProcessConfigTable(db);
    insertRow(db, "routine.morning_routine_today", 2.0, "preset", "claude");
    runMigrations(db, [migration!]);
    expect(budgetOf(db, "routine.morning_routine_today")).toBe(2.0);
  });

  it("bumps a claude preset-default morning_routine_today row from $1.50 to $2.00", () => {
    const db = openDb();
    seedProcessConfigTable(db);
    insertRow(db, "routine.morning_routine_today", 1.5, "preset", "claude");
    const result = runMigrations(db, [migration!]);
    expect(result.applied).toEqual(["0025-morning-today-budget-bump"]);
    expect(budgetOf(db, "routine.morning_routine_today")).toBe(2.0);
  });

  it("bumps an opencode preset-default morning_routine_today row from $1.50 to $2.00", () => {
    const db = openDb();
    seedProcessConfigTable(db);
    insertRow(db, "routine.morning_routine_today", 1.5, "preset", "opencode");
    runMigrations(db, [migration!]);
    expect(budgetOf(db, "routine.morning_routine_today")).toBe(2.0);
  });

  it("bumps a codex preset row from the scaled $2.25 to $3.00", () => {
    const db = openDb();
    seedProcessConfigTable(db);
    insertRow(db, "routine.morning_routine_today", 2.25, "preset", "codex");
    runMigrations(db, [migration!]);
    expect(budgetOf(db, "routine.morning_routine_today")).toBe(3.0);
  });

  it("bumps a gemini preset row from the scaled $2.25 to $3.00", () => {
    const db = openDb();
    seedProcessConfigTable(db);
    insertRow(db, "routine.morning_routine_today", 2.25, "preset", "gemini");
    runMigrations(db, [migration!]);
    expect(budgetOf(db, "routine.morning_routine_today")).toBe(3.0);
  });

  it("does not bump a codex preset row sitting at the claude $1.50 band", () => {
    const db = openDb();
    seedProcessConfigTable(db);
    insertRow(db, "routine.morning_routine_today", 1.5, "preset", "codex");
    runMigrations(db, [migration!]);
    expect(budgetOf(db, "routine.morning_routine_today")).toBe(1.5);
  });

  it("leaves a codex operator-pinned ('user') row untouched even at $2.25", () => {
    const db = openDb();
    seedProcessConfigTable(db);
    insertRow(db, "routine.morning_routine_today", 2.25, "user", "codex");
    runMigrations(db, [migration!]);
    expect(budgetOf(db, "routine.morning_routine_today")).toBe(2.25);
  });

  it("leaves operator-pinned ('user') rows untouched even at $1.50", () => {
    const db = openDb();
    seedProcessConfigTable(db);
    insertRow(db, "routine.morning_routine_today", 1.5, "user");
    runMigrations(db, [migration!]);
    expect(budgetOf(db, "routine.morning_routine_today")).toBe(1.5);
  });
  ...
  it("leaves a preset row already at a custom value untouched", () => {
    const db = openDb();
    seedProcessConfigTable(db);
    insertRow(db, "routine.morning_routine_today", 4.0, "preset");
    runMigrations(db, [migration!]);
    expect(budgetOf(db, "routine.morning_routine_today")).toBe(4.0);
  });

  it("does not touch sibling routine rows (routine.evening_review at the same $1.50 band)", () => {
    const db = openDb();
    seedProcessConfigTable(db);
    insertRow(db, "routine.evening_review", 1.5, "preset", "codex");
    runMigrations(db, [migration!]);
  ...
  it("is idempotent — re-running does not bump again", () => {
    const db = openDb();
    seedProcessConfigTable(db);
    insertRow(db, "routine.morning_routine_today", 1.5, "preset");
    runMigrations(db, [migration!]);
    const second = runMigrations(db, [migration!]);
    expect(second.applied).toEqual([]);
    expect(budgetOf(db, "routine.morning_routine_today")).toBe(2.0);
