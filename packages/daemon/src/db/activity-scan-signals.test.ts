...
describe("computeActivityScanSignals", () => {
  ...
  it("counts VIP unread mail using case-insensitive exact match", () => {
    ...
    const signals = computeActivityScanSignals(db, {
      now,
  ...
  it("counts overdue Agent Plan rows from today.md", () => {
    ...
    const signals = computeActivityScanSignals(db, { now, todayMd });
  ...
  it("falls back to legacy routine.activity_scan rows when no gate row exists", () => {
    ...
    db.prepare(
      `INSERT INTO agent_actions (action_type, result, started_at, completed_at)
       VALUES ('routine.activity_scan', 'success', ?, ?)`,
