...
function seedVault(dir: string): void {
  ...
  writeFileSync(join(dir, "rules", "management.md"), "# mgmt\n");
}
...
describe("POST /api/setup/migrate-context", () => {
  ...
  it("manual reseed skips rules/management before setup completes", async () => {
    ...
    mkdirSync(resolve(templatesRoot, "rules"), { recursive: true });
    ...
    writeFileSync(resolve(templatesRoot, "rules", "management.md"), managementTemplate);
    writeFileSync(resolve(templatesRoot, "rules", "redaction.md"), redactionTemplate);
    ...
    expect(existsSync(resolve(contextDir, "rules", "management.md"))).toBe(false);
    expect(existsSync(resolve(contextDir, "rules", "redaction.md"))).toBe(true);
  });
  ...
  it("manual reseed includes rules/management after setup completes", async () => {
    ...
    expect(res.status).toBe(200);
    expect(readFileSync(resolve(contextDir, "rules", "management.md"), "utf-8")).toBe(
      managementTemplate,
    );
  });
  ...
  it("seeds the skeleton into the target but skips rules/management before setup completes", async () => {
    ...
    const templatesRoot = resolve(workspaceDir, "agent-assets", "templates");
    mkdirSync(resolve(templatesRoot, "rules"), { recursive: true });
    mkdirSync(resolve(templatesRoot, "user"), { recursive: true });
    ...
    writeFileSync(
      resolve(templatesRoot, "rules", "management.md"),
      managementTemplate,
    );
    writeFileSync(
      resolve(templatesRoot, "user", "profile.md"),
      profileTemplate,
    );
    ...
    // already set up.
    expect(existsSync(resolve(targetDir, "rules", "management.md"))).toBe(false);
    expect(readFileSync(resolve(targetDir, "user", "profile.md"), "utf-8")).toBe(
      profileTemplate,
    );
    // Placeholders that `ensureSkeletonFiles` writes unconditionally.
    expect(existsSync(resolve(targetDir, "today.md"))).toBe(true);
    expect(existsSync(resolve(targetDir, "roadmap.md"))).toBe(true);
  });
  ...
  it("seeds rules/management during migration after setup has completed", async () => {
    ...
    expect(res.status).toBe(200);
    expect(readFileSync(resolve(targetDir, "rules", "management.md"), "utf-8")).toBe(
      managementTemplate,
    );
  });
  ...
  it("preserves existing vault content when re-targeting via migrate-context", async () => {
    ...
    const migrated = readFileSync(
      resolve(targetDir, "rules", "management.md"),
      "utf-8",
    );
