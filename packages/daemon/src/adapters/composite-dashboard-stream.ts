 ...
 *
 * The dispatcher knows one outbound stream at a time
 * (`setDashboardStream(adapter)`), but the docs-QA pipeline needs a
 * second dashboard-platform adapter (`DocsQAAdapter`) to receive the
 * same outbound calls. Channel routing is content-addressed by
 * `channelId` (each underlying adapter no-ops on unknown ids), so
 * fan-out is naturally safe — only the adapter that owns the channel
 * actually emits.
 *
 ...
 * optional method, so we guard with `s.method?.(...)`.
 */
