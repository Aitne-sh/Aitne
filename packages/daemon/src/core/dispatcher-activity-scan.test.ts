...
function fakeConfig(dataDir: string): AgentConfig {
  return {
    ...
    timezone: "UTC",
    dayBoundaryHour: 4,
    activityScanMinObservations: 5,
    activityScanHeartbeatHours: 4,
    activityScanStage2Enabled: false,
    activityScanLowSignalPendingCeiling: 0,
    activityScanPrePassFreshnessMinutes: 30,
    vipMailSenders: [],
...
function makeStubFetchWindowRunner(): RoutineFetchWindowRunner {
  return {
    run: vi.fn().mockResolvedValue({
      ...
      block: `<fetch_report routine="activity_scan" agent_day="2026-05-17" status="skipped" fetched="0" posted="0" duplicates="0" />`,
    }),
  } as unknown as RoutineFetchWindowRunner;
}

interface CoordinatorHandle {
  coordinator: ActivityScanCoordinator;
  eventBus: EventBus;
  state: {
    activityScanInProgress: boolean;
    morningRoutineActive: boolean;
    autonomousBlock: TriggerActivityScanSkipReason | null;
  };
  audit: IAuditLogger;
  router: IAgentRouter;
}
...
function makeCoordinator(opts: {
  ...
  dataDir: string;
  config?: AgentConfig;
  initialAutonomousBlock?: TriggerActivityScanSkipReason | null;
  ...
  const coordinator = new ActivityScanCoordinator({
    db: opts.db,
    config,
    eventBus,
    contextBuilder,
    ...
    setActivityScanInProgress: (value) => {
      state.activityScanInProgress = value;
    },
    isActivityScanInProgress: () => state.activityScanInProgress,
...
describe("ActivityScanCoordinator — early skip gates", () => {
  ...
  it("skips with reason=activity_scan_in_progress when the flag is already set", async () => {
    const { coordinator, state } = makeCoordinator({ db, dataDir });
    state.activityScanInProgress = true;
    const result = await coordinator.trigger("cron");
    expect(result).toMatchObject({
      status: "skipped",
      reason: "activity_scan_in_progress",
    });
    expect(state.activityScanInProgress).toBe(true);
  });
  ...
  it("skips with reason from isAutonomousAllowed when the gate forbids autonomous work", async () => {
    ...
    expect(state.activityScanInProgress).toBe(false);
  });
...
describe("ActivityScanCoordinator — morning_routine_pending_for_today gate", () => {
  let db: Database.Database;
  let dataDir: string;
  ...
  it("skips and enqueues a morning_routine wake when no successful morning_routine row exists", async () => {
    ...
    expect(queueWake.mock.calls[0][0]).toBe("activity_scan_dependency:cron");
    expect(state.activityScanInProgress).toBe(false);
  });
...
// (`activityScanPrePassFreshnessMinutes`, default 30 min) prevents redundant
// fetches; `forced` runs bypass the window.
// ────────────────────────────────────────────────────────────────────────────

describe("ActivityScanCoordinator — Layer-1 pre-pass harvest", () => {
  let db: Database.Database;
  let dataDir: string;
  ...
  function makeSuccessRunner(): {
    ...
    const run = vi.fn().mockResolvedValue({
      ...
      block:
        '<fetch_report routine="activity_scan" agent_day="2026-05-17" status="success" fetched="1" posted="1" duplicates="0" />',
    });
    return {
  ...
  it("skips pre-pass entirely when no non-direct integration is active", async () => {
    ...
    (config as { activityScanMinObservations: number }).activityScanMinObservations = 0;
    (config as { activityScanHeartbeatHours: number }).activityScanHeartbeatHours = 24;
  ...
  it("attaches the pre-pass <fetch_report> block onto the Stage 3 event", async () => {
    ...
    const config = fakeConfig(dataDir);
    (config as { activityScanMinObservations: number }).activityScanMinObservations = 0;
    (config as { activityScanHeartbeatHours: number }).activityScanHeartbeatHours = 0;
    const { coordinator, eventBus } = makeCoordinator({
      db,
      dataDir,
      config,
  ...
  it("§3.5 cautious-escalate: pre-pass failure forces stage3 regardless of signal verdict", async () => {
    ...
    const run = vi.fn().mockResolvedValue({
      ...
      block:
        '<fetch_report routine="activity_scan" agent_day="2026-05-17" status="failed" fetched="0" posted="0" duplicates="0" />',
    });
    ...
    (config as { activityScanMinObservations: number }).activityScanMinObservations = 0;
    (config as { activityScanHeartbeatHours: number }).activityScanHeartbeatHours = 999;
    const { coordinator, eventBus } = makeCoordinator({
      db,
      dataDir,
      config,
    ...
    const auditRow = db
      .prepare(
        "SELECT detail FROM agent_actions WHERE action_type='activity_scan.gate' ORDER BY id DESC LIMIT 1",
      )
...
describe("ActivityScanCoordinator — mode-blind gate signals", () => {
  let db: Database.Database;
  let dataDir: string;
  ...
  it("counts actor='agent' delegated rows toward pendingCount (defect fix)", async () => {
    ...
    const config = fakeConfig(dataDir);
    (config as { activityScanMinObservations: number }).activityScanMinObservations = 1;
    (config as { activityScanHeartbeatHours: number }).activityScanHeartbeatHours = 24;
...
describe("ActivityScanCoordinator — forced enqueue mechanics", () => {
  let db: Database.Database;
  let dataDir: string;
  ...
  it("skips with reason=below_threshold when pendingCount<min and gate stage is stage3 (legacy floor)", async () => {
    ...
    const config = fakeConfig(dataDir);
    (config as { activityScanMinObservations: number }).activityScanMinObservations = 5;
    (config as { activityScanHeartbeatHours: number }).activityScanHeartbeatHours = 0;
    const { coordinator } = makeCoordinator({ db, dataDir, config });
    const result = await coordinator.trigger("cron");
    expect(result).toMatchObject({
  ...
  it("queues Stage 3 with forced=true regardless of pendingCount", async () => {
    ...
    const event = await eventBus.get();
    expect((event as { type: string }).type).toBe("routine.activity_scan");
    expect(state.activityScanInProgress).toBe(true);
  });
});
