...
import {
  ...
  type HourlyCheckGateDecision,
  type HourlyCheckGateStage,
} from "../scheduler/hourly-check-gate.js";
...
export class HourlyCheckCoordinator {
  ...
  private readonly setHourlyCheckInProgress: (value: boolean) => void;
  private readonly isHourlyCheckInProgress: () => boolean;
  private readonly isMorningRoutineActive: () => boolean;
  private readonly isAutonomousAllowed: () => TriggerHourlyCheckSkipReason | null;
    ...
    try {
      ...
      if (setupBlock !== null) {
        logger.info(
          ...
          "Hourly check skipped — autonomous work paused for setup",
        );
        return {
          status: "skipped",
          reason: setupBlock,
      ...
      if (!morningRoutineRanToday(this.db, this.config)) {
        ...
        if (queueWake) {
          ...
          logger.info(
            { source, queueResult },
            "Hourly check skipped — morning_routine not yet complete for current agent-day; enqueued morning_routine wake",
          );
        } else {
          logger.warn(
            { source },
            "Hourly check skipped — morning_routine not yet complete and queueMorningRoutineWake not wired",
          );
        }
        return {
          status: "skipped",
      ...
      if (delegatedSyncRefresh) {
        try {
          ...
          logger.warn(
            { err, source },
            "Pre-hourly-check delegated sync refresh failed; proceeding with stale snapshot",
          );
        }
      }
      ...
      const decision: HourlyCheckGateDecision = cautiousEscalate
        ? {
            ...baseDecision,
            stage: "stage3",
      ...
      if (decision.stage === "stage2") {
        ...
        if (verdict === "log_only") {
          const silentResult = this.runSilentHourlyCheckPath(
            decision,
            "stage2_log_only",
        ...
        await this.enqueueStage3HourlyCheck(
          source,
          decision,
          {
            forced,
  ...
  private async harvestForGate(
    ...
    const freshnessMinutes =
      this.config.hourlyCheckPrePassFreshnessMinutes ?? 30;
   ...
   */
  private computeHourlyCheckGateDecision(): HourlyCheckGateDecision {
  ...
  private runSilentHourlyCheckPath(
    decision: HourlyCheckGateDecision,
    appliedDecision: "stage0_silent" | "stage2_log_only",
    ctx: {
      source: string;
      forced: boolean;
      harvest: HarvestResult;
      preEscalate: { stage: HourlyCheckGateStage; reason: string } | null;
    },
  ): { status: "skipped"; reason: TriggerHourlyCheckSkipReason; pendingCount: number; forced: boolean } {
    const reason: TriggerHourlyCheckSkipReason =
      ...
      try {
        ...
        if (pending.length > 0) {
          consumeObservations(
            ...
            `hourly_check_gate:${appliedDecision}`,
          );
        }
      } catch (err) {
  ...
  private async enqueueStage3HourlyCheck(
    source: string,
    decision: HourlyCheckGateDecision,
    extra: {
      forced: boolean;
  ...
  private logGateAuditRow(
    ...
    try {
      ...
      // merged detail.
      const auditAppliedDecision: HourlyCheckGateStage =
        params.appliedDecision === "stage2_log_only"
          ? "stage0_silent"
          : (params.appliedDecision as HourlyCheckGateStage);
      const detail = {
  ...
  private async runStage2Triage(
    ...
    const processKey: ProcessKey = "routine.hourly_check.triage";
