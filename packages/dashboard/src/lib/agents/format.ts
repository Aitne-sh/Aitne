...
/**
 * Compact "just now" / "5m ago" / "2h ago" / "3d ago" relative time for a past
 * ISO timestamp; `null` when absent or unparseable. `now` is injectable (epoch
 * ms) so this stays a deterministic, directly-unit-testable transform — it
 * defaults to the wall clock for callers. Future timestamps collapse to
 * "just now" (a clock skew, not something to surface).
 */
